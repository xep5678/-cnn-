...
def data_load_train_1(file_path, length_freq, SNR_range, num_read_sources, num_of_receiver):
    ...
    for v in mask:
        # np.sum(rand_file_index == v)数的是rand_file_index取值为v的个数有多少个
        ...
        for iv in range(0, tp):  # Read individual sound sources
            ...
            jump = int(RN[iv])
            #fileObject.seek(offset[, whence]):offset是偏移的字节数，whence指的是起始位置
            #个人预估，这里fid.seek()指的是，在文件中随机找一个距离点。
            fid.seek(jump * size_data_one_source * 4, 0)
            #.unpack('f')指的是以float形式解码。
            ...
            #.conj()是求共轭,注意：这里是对pres2求共轭，然后让pres1乘上这个共轭结果。
            Rx = (pres1 * pres2.conj())
...
def data_load_test_s_1(file_path, length_freq, Sr, Sd, SNR, i_file=0, num_of_receiver=13):
    ...
    sigma_noise = 10 ** (-SNR / 20) / np.sqrt(num_of_receiver)  # Gaussian noise standard deviation(高斯噪声标准差)
    fid = open(file_path, 'rb')
